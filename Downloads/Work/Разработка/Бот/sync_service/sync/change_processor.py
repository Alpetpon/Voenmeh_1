...
class ChangeProcessor:
    ...
    async def _handle_insert(self, change: Dict[str, Any], table_config: TableConfig) -> bool:
        ...
        try:
            ...
            # Получаем cabinet из JSON данных
            cabinet = new_data.get('cabinet')
            if not cabinet:
                logger.warning(f"No cabinet in change data, skipping. Change keys: {list(change.keys())}, new_data keys: {list(new_data.keys()) if new_data else 'None'}")
                return False
            
            # Получаем информацию о клиенте из tech таблицы
            client_sheets_client = await self._get_client_sheets_client(cabinet)
            if not client_sheets_client:
                logger.warning(f"No client sheets client found for cabinet {cabinet}")
                return False
            
            # Конвертируем данные в формат Sheets
            sheet_data = client_sheets_client.convert_to_sheet_format(
            ...
            # Добавляем строку в конец листа в таблице клиента
            await client_sheets_client.append_sheet_data(
                table_config.sheet_name, sheet_data
            )
            
            logger.debug(f"Inserted record {change['row_id']} to sheet {table_config.sheet_name} for client {cabinet}")
            return True
    ...
    async def _handle_update(self, change: Dict[str, Any], table_config: TableConfig) -> bool:
        ...
        try:
            ...
            # Получаем данные из листа в таблице клиента
            sheet_data = await client_sheets_client.get_sheet_data(table_config.sheet_name)
            ...
            updated_row = client_sheets_client.convert_to_sheet_format(
                [new_data], table_config.fields, include_headers=False
            )
            
            if updated_row:
                range_name = f"A{target_row_index}:{chr(65 + len(updated_row[0]) - 1)}{target_row_index}"
                await client_sheets_client.update_sheet_data(
    ...
    async def _handle_delete(self, change: Dict[str, Any], table_config: TableConfig) -> bool:
        ...
        try:
            ...
            # Получаем cabinet из JSON данных
            cabinet = old_data.get('cabinet')
            if not cabinet:
                logger.warning(f"No cabinet in change data, skipping. Change keys: {list(change.keys())}, old_data keys: {list(old_data.keys()) if old_data else 'None'}")
                return False
    ...
    async def _get_client_sheets_client(self, cabinet: str) -> Optional[GoogleSheetsClient]:
        """Получить клиент Google Sheets для конкретного cabinet"""
        try:
            # Получаем данные из tech таблицы
            tech_data = await self.sheets_client.get_sheet_data('tech')
            
            if not tech_data:
                return None
            
            # Ищем строку с нужным cabinet (cabinet это код клиента в tech таблице)
            for row in tech_data[1:]:  # Пропускаем заголовок
                if len(row) >= 2 and row[0] == cabinet:
                    client_table_id = row[1]
                    
                    # Создаем клиент для таблицы клиента
                    client_sheets = GoogleSheetsClient(
                        credentials_file=self.config_loader.load_env_config()['google_sheets'].credentials_file,
                        spreadsheet_id=client_table_id
                    )
            ...
            logger.error(f"Error getting client sheets client for cabinet {cabinet}: {e}")
            return None
